...
class Beat:
    ...
    def ante(self, players, first_player): # TODO: Returns list of applied antes, split Apply and PayCost, so we can apply to clashed pairs
    ...
    def handleClashes(self, players):
        p0_bases_hand = deepcopy(players[0].bases_hand)
        p0_styles_hand = deepcopy(players[0].styles_hand)
        p1_bases_hand = deepcopy(players[1].bases_hand)
        p1_styles_hand = deepcopy(players[1].styles_hand)
        # print(self.played_pairs)

        while self.played_pairs[0].priority == self.played_pairs[1].priority and len(p0_bases_hand) > 1 and len(p0_styles_hand) > 1 and len(p1_bases_hand) > 1 and len(p1_styles_hand) > 1:
            # print("Clash.")
            p0_bases_hand = [card for card in p0_bases_hand if card.name != self.played_pairs[0].base.name]
            p0_styles_hand = [card for card in p0_styles_hand if card.name != self.played_pairs[0].style.name]
            p1_bases_hand = [card for card in p1_bases_hand if card.name != self.played_pairs[1].base.name]
            p1_styles_hand = [card for card in p1_styles_hand if card.name != self.played_pairs[1].style.name]
            new_p0_pair = players[0].choosePair(action_name="play", bases_hand=p0_bases_hand, styles_hand=p0_styles_hand)
            new_p1_pair = players[1].choosePair(action_name="play", bases_hand=p1_bases_hand, styles_hand=p1_styles_hand)
            self.played_pairs = [new_p0_pair, new_p1_pair]
            # print(self.played_pairs)
